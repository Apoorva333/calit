...
*Post-Plan-3 enhancement (migration V5):*
- `MeetingType` gains `slotIntervalMinutes` (`Integer`, nullable; column `slot_interval_minutes`) — Calendly-style slot cadence: minutes between consecutive slot START times. `null`/≤0 → `MeetingType.effectiveSlotIntervalMinutes()` falls back to `durationMinutes` (back-to-back, the original behavior). `SlotService.generateRawSlots` steps each window by `effectiveSlotIntervalMinutes()`; interval > duration leaves gaps, interval < duration emits overlapping candidate starts (Plan 3 busy/buffer subtraction resolves conflicts). **Plan 5 admin UI should expose this per-type field.**
