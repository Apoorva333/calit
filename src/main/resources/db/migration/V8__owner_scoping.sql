...
-- The V2 uniqueness (COALESCE(meeting_type_id,0), override_date) was global and now blocks two owners
-- from each having a global override on the same date. Make it per-owner.
DROP INDEX uq_date_override_scope_date;
CREATE UNIQUE INDEX uq_date_override_scope_date
    ON date_override (owner_id, COALESCE(meeting_type_id, 0), override_date);
...
-- Enforce owner_id NOT NULL at the DB level. The entities map it nullable=false, but Hibernate runs
-- with schema-management=validate (it never alters the schema), so without this the column would stay
-- nullable and a null-owner row could persist. Tables are empty here (fresh start; the V1 global
-- booking_field seed was deleted above), so SET NOT NULL succeeds. ON DELETE CASCADE already removes a
-- user's rows, so NOT NULL never blocks user deletion.
ALTER TABLE owner_settings     ALTER COLUMN owner_id SET NOT NULL;
ALTER TABLE meeting_type       ALTER COLUMN owner_id SET NOT NULL;
ALTER TABLE booking            ALTER COLUMN owner_id SET NOT NULL;
ALTER TABLE booking_field      ALTER COLUMN owner_id SET NOT NULL;
ALTER TABLE availability_rule  ALTER COLUMN owner_id SET NOT NULL;
ALTER TABLE date_override      ALTER COLUMN owner_id SET NOT NULL;
ALTER TABLE google_credential  ALTER COLUMN owner_id SET NOT NULL;
ALTER TABLE google_calendar    ALTER COLUMN owner_id SET NOT NULL;
